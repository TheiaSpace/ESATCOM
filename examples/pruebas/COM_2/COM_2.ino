...
void setup()
{
  ...
  ESAT_COM.begin(radioInputBuffer,
                ...
                (unsigned long) WHOLE_PACKET_BUFFER_LENGTH);
  interrupts();
  ReceptionTransceiver.setChannel(32);
...
void loop()
{
  if (iterationCounter >= PERIOD)
  {
    ...
    packet.writeByte('J');
    packet.writeByte('T');
    packet.print(ProcessorTemperature.read(), 2);
    packet.println("En un lugar de la Mancha, de cuyo nombre no quiero acordarme, no ha mucho tiempo que vivía un hidalgo de los de lanza en astillero, adarga antigua, rocín flaco y galgo corredor.");
